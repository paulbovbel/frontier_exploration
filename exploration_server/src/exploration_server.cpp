...
namespace exploration_server {
    ...
    ExplorationServer::ExplorationServer(ros::NodeHandle nh, ros::NodeHandle private_nh) :
        nh_(nh), private_nh_("~"), retry_(5), tf_listener_(), tf2_listener_(tf2_buffer_),
        success_(false), moving_(false), move_client_("move_base", true),
        previous_state_(actionlib::SimpleClientGoalState::PENDING),
        explore_action_server_(nh, "exploration_server_node",
                               boost::bind(&ExplorationServer::goalCb, this, _1),
                               boost::bind(&ExplorationServer::cancelGoalCb, this, _1), false)
    {
    ...
    void ExplorationServer::goalCb(GoalHandle gh)
    {
        ...
        pluginlib::ClassLoader<exploration_server::BasePlugin> planner_loader(
            "exploration_server", "exploration_server::BasePlugin");
        try
        {
        ...
        // make use of the polygon service to set the boundary on the polygon using what is stored
        // in the goalhandle
    ...
    bool ExplorationServer::inBoundary()
    {
        ...
        // check if robot is not within exploration boundary and needs to return to center of search
        // area
        if (polygon_.polygon.points.size() > 0 &&
            !pointInPolygon(eval_pose.pose.position, polygon_.polygon))
        {
            ...
            goal_pose.pose.orientation = tf::createQuaternionMsgFromYaw(
                yawOfVector(eval_point.point, explore_center_.point));
            ...
            move_client_.sendGoal(move_client_goal_,
                                  boost::bind(&ExplorationServer::moveBaseResultCb, this, _1, _2),
                                  0, 0);
            moving_ = true;
            lock.unlock();
            return false;
    ...
    void ExplorationServer::requestAndSendGoal()
    {
        ...
        point_list = planner_->whereToExplore(
            current_pose, move_client_goal_.target_pose.pose.position, previous_state_);
        ...
        if (point_list.size() != 0)
        {
            ...
            float dist_to_robot;
            ROS_INFO("Robot is now at %f, %f", current_pose.pose.position.x,
                     current_pose.pose.position.y);
            for (const auto& point : point_list)
            {
                // calculate the distance between a given point and the robot location
                dist_to_robot = sqrt(pow(current_pose.pose.position.x - point.x, 2) +
                                     pow(current_pose.pose.position.y - point.y, 2));
            ...
            next_goal.pose.orientation = tf::createQuaternionMsgFromYaw(
                yawOfVector(current_pose.pose.position, next_goal.pose.position));
            { // scope the mutex lock
                ...
                move_client_.sendGoal(
                    move_client_goal_,
                    boost::bind(&ExplorationServer::moveBaseResultCb, this, _1, _2), 0, 0);
                moving_ = true;
                lock.unlock();
            }
        } else
    ...
    void ExplorationServer::start() { explore_action_server_.start(); }

...
namespace exploration_server {
    ...
    void PluginClient::pointCb(const geometry_msgs::PointStampedConstPtr& point)
    {
        ...
        if (waiting_for_center_)
        {
            ...
            {
                actionlib::SimpleActionClient<exploration_msgs::ExploreAction> exploreClient(
                    "exploration_server_node", true);
    ...
    PluginClient::PluginClient() : nh_(), private_nh_("~"), waiting_for_center_(false)
    {
        ...
        point_viz_pub_ =
            nh_.advertise<visualization_msgs::Marker>("exploration_polygon_marker", 10);
        point_viz_timer_ =
            nh_.createWallTimer(ros::WallDuration(0.1), boost::bind(&PluginClient::vizPubCb, this));

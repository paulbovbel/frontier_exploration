...
namespace exploration_server {
    ...
    void ExamplePlugin::addToVisited(geometry_msgs::Point previous_goal,
    ...
    ExamplePlugin::whereToExplore(const geometry_msgs::PoseStamped& start_pose,
                                  ...
                                  const actionlib::SimpleClientGoalState& state)
    {
        ...
        for (const auto& point : point_list)
        {
            ...
            dist_to_robot = sqrt(pow(start_pose.pose.position.x - point.x, 2) +
                                 pow(start_pose.pose.position.y - point.y, 2));
            // if it is one of the closest points to the robot (within the threshold) and hasn't
            // been visited send it

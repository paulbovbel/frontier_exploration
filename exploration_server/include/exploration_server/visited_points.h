...
namespace exploration_server {
    ...
    /**
     * @brief returns a hash value for the boost_point combining point values from x and y
     * coordinates
     */
    size_t point_hasher(boost_point const& k)
    {
        ...
        // Modify 'seed' using boost hash_combine to combine the boost hash_values for the x and y
        // coordinates
    ...
    private:
        ...
        std::unordered_map<boost_point, actionlib::SimpleClientGoalState::StateEnum,
                           size_t (*)(const boost_point&),
                           bool (*)(const boost_point&, const boost_point&)>
            visited_{0, point_hasher, boost::geometry::equals};
         ...
         */
        std::vector<boost_point> getPointsNear(const geometry_msgs::Point& point,
         ...
         * @return actionlib state that was returned last time the robot tried to navigate to a
         * point in that boost_box
         */
    ...
    public:
         ...
         * @param key Point to look up and determine if the robot succeeded or aborted movement to
         * it
         * @param query_width half of width of box to query for points nearby
         * @return ABORTED if the point lies in any ABORTED box, SUCCEEDED if it lies in a SUCCEEDED
         * box and not an aborted box, and PENDING otherwise
         */

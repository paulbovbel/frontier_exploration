...
namespace exploration_server {
    ...
    double polygonPerimeter(const geometry_msgs::Polygon& polygon)
    {
        ...
        {
            for (unsigned int i = 0, j = polygon.points.size() - 1; i < polygon.points.size();
                 j = i++)
            {
    ...
    /**
     * @brief Evaluate whether a point is approximately adjacent, within a specified proximity
     * distance, to any point in a list.
     * @param one Point one
    ...
    bool pointInPolygon(const T& point, const geometry_msgs::Polygon& polygon)
    {
        ...
        for (unsigned int i = 0, j = polygon.points.size() - 1; i < polygon.points.size(); j = i++)
        {
            if (((polygon.points[i].y > point.y) != (polygon.points[j].y > point.y)) &&
                (point.x < (polygon.points[j].x - polygon.points[i].x) *

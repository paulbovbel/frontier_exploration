...
namespace polygon_layer {
    ...
    void PolygonLayer::onInitialize()
    {
        ...
        set_polygon_service_ =
            nh_.advertiseService("set_polygon", &PolygonLayer::setPolygonCb, this);
        current_polygon_pub_ =
            nh_.advertise<geometry_msgs::PolygonStamped>("current_polygon", 10, true);

        dsrv_ =
            boost::make_shared<dynamic_reconfigure::Server<costmap_2d::GenericPluginConfig>>(nh_);
        dynamic_reconfigure::Server<costmap_2d::GenericPluginConfig>::CallbackType cb =
            boost::bind(&PolygonLayer::reconfigureCb, this, _1, _2);
    ...
    bool PolygonLayer::setPolygonCb(exploration_msgs::SetPolygon::Request& req,
                                    exploration_msgs::SetPolygon::Response& res)
    {
    ...
    bool PolygonLayer::setPolygon(const geometry_msgs::PolygonStamped& polygon)
    {
        ...
        } else
        {
            ...
            if (!tf_listener_.waitForTransform(layered_costmap_->getGlobalFrameID(),
                                               polygon.header.frame_id, ros::Time::now(),
                                               ros::Duration(.1)))
            {
                ROS_ERROR_STREAM("Couldn't transform from " << layered_costmap_->getGlobalFrameID()
                                                            << " to " << polygon.header.frame_id);
                return false;
            } else
            {
                                    ...
                                    double* min_y, double* max_x, double* max_y)
    {
        ...
        *max_y = max_y_;
        ...
        for (unsigned int i = 0, j = polygon_.polygon.points.size() - 1;
             i < polygon_.polygon.points.size(); j = i++)
        {
    ...
    void PolygonLayer::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j,
                                   int max_i, int max_j)
    {
        if (!enabled_)
            return;
        ...
        for (int j = min_j; j < max_j; j++)
        {
            for (int i = min_i; i < max_i; i++)
            {
                ...
                if (master[index] != LETHAL_OBSTACLE &&
                    (costmap_[index] == LETHAL_OBSTACLE || costmap_[index] > master[index]))

...
namespace frontier_exploration {
    ...
    FrontierSearch::FrontierSearch(costmap_2d::Costmap2D& costmap, unsigned int min_frontier_size,
        ...
        costmap_(costmap),
        min_frontier_size_(min_frontier_size), travel_point_(travel_point)
    {
    }
    ...
    std::list<Frontier> FrontierSearch::searchFrom(geometry_msgs::Point position)
    {
        ...
        while (!bfs.empty())
        {
            ...
            {
                // add to queue all free, unvisited cells, use descending search in case initialized
                // on non-free cell
        ...
        while (!bfs.empty())
        {
            ...
            BOOST_FOREACH (unsigned int nbr, nhood8(idx, costmap_))
            {
                ...
                if (isNewFrontierCell(nbr, frontier_flag))
                {
                    ...
                    double distance =
                        exploration_server::distanceBetweenCoords(wx, reference_x, wy, reference_y);

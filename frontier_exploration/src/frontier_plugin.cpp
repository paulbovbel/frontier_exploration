...
namespace frontier_exploration {
    ...
    void FrontierPlugin::clearBlacklistVisually()
    {
        ...
        marker.type = visualization_msgs::Marker::CUBE;
        marker.ns = "blacklist";
        // The constant does not exist in ROS Indigo, although functionality is implemented. We use
        // our own.
        marker.action = DELETEALL;
    }

    void FrontierPlugin::addToVisited(geometry_msgs::Point previous_goal,
    ...
    FrontierPlugin::whereToExplore(const geometry_msgs::PoseStamped& start_pose,
                                   const geometry_msgs::Point& previous_goal,
                                   const actionlib::SimpleClientGoalState& state)
    {
        ...
        if (start_pose.header.frame_id != explore_costmap_ros_->getGlobalFrameID())
        {
            ...
            if (!tf_listener_.waitForTransform(explore_costmap_ros_->getGlobalFrameID(),
                                               start_pose.header.frame_id, ros::Time::now(),
                                               ros::Duration(.1)))
            {
                ROS_ERROR_STREAM("Couldn't transform from "
                                 << explore_costmap_ros_->getGlobalFrameID() << " to "
                                 << start_pose.header.frame_id);
                return result;
            }
            geometry_msgs::PoseStamped temp_pose = start_pose;
            tf_listener_.transformPose(explore_costmap_ros_->getGlobalFrameID(), temp_pose,
                                       tf_start_pose);
        }

        // initialize frontier search implementation
        FrontierSearch frontierSearch(*(explore_costmap_ros_->getCostmap()), min_frontier_size_,
